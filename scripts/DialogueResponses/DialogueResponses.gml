			...
			switch(argument0)
					{
						...
						//Mãe
							...
							case 7:
									{		
	    								ScreenShake(20, 3, 0.3)  //Tremor ma tela 
						...
						//Configruações de cenas e aúdio
							RoomTransition(TRANS_TYPE.FADE, rCreditos) //Transição de cena para os créditos
							audio_sound_gain(aSalCo,0,1000);           //Música do ambiente Sala e cozinha
							if(audio_sound_get_gain(aSalCo)<= 0)       //Parar Música ambiente
			         {
				       audio_stop_sound(aSalCo);
			         }
							audio_play_sound(aCreditos,1000,true);     //Música dos créditos
							audio_sound_gain(aCreditos,0,0);           //aumento gradual da música dos créditos 
							audio_sound_gain(aCreditos,1,1000);        //Diminuição gradual da música dos créditos
